...
Uses the new preferred names (Returns, Raises, Loop, DoTry) alongside
the old names to verify both work for type checking.
"""
...
# return with kwargs
sb.returns("r1", value="ok")

# return with lambda
sb.returns("r2", lambda r: r.value("ok"))

# raise with kwargs
sb.raises("e1", value="err")

# raise with lambda
sb.raises("e2", lambda r: r.value("err"))
...
# for with kwargs
sb.loop("f1", value="item", in_=["a", "b"], steps=StepBuilder())

# for with lambda + value kwarg
sb.loop("f2", lambda f: f.items(["a"]).steps(StepBuilder()), value="item")
...
# try with kwargs
sb.do_try("t1", body=StepBuilder())

# try with lambda
sb.do_try("t2", lambda t: t.body(StepBuilder()))
