...
# =============================================================================
# Alias methods (.assign, .call, .returns, .raises, .switch, .loop,
#                .parallel, .do_try, .nested, .steps)
# =============================================================================
...
class TestAssignAlias:
    ...
    def test_simple_kwargs(self):
        ...
        assert _to_dict(s) == [{"init": {"assign": [{"x": 10}, {"y": 20}]}}]

    def test_with_mapping_dict(self):
        s = Steps().assign("init", {"x": 10, 'map["key"]': "value"})
        assert _to_dict(s) == [
            {"init": {"assign": [{"x": 10}, {'map["key"]': "value"}]}}
        ]
    ...
    def test_with_expr(self):
        ...
        assert _to_dict(s) == [{"init": {"assign": [{"x": 10}, {"y": "${x + 1}"}]}}]

    def test_with_next(self):
        s = Steps().assign("init", x=10, next="done")
        assert _to_dict(s) == [{"init": {"assign": [{"x": 10}], "next": "done"}}]
...
class TestCallAlias:
    ...
    def test_simple_call(self):
        ...
        assert _to_dict(s) == [{"log": {"call": "sys.log", "args": {"text": "hello"}}}]
    ...
    def test_with_next(self):
        ...
        assert _to_dict(s) == [
            {"log": {"call": "sys.log", "args": {"text": "hi"}, "next": "done"}}
        ]

    def test_no_args(self):
        s = Steps().call("invoke", "my_subworkflow")
        assert _to_dict(s) == [{"invoke": {"call": "my_subworkflow"}}]
...
class TestReturnsAlias:
    ...
    def test_string_value(self):
        ...
        assert _to_dict(s) == [{"done": {"return": "ok"}}]

    def test_expr_value(self):
        s = Steps().returns("done", expr("x + y"))
        assert _to_dict(s) == [{"done": {"return": "${x + y}"}}]
    ...
    def test_dict_value(self):
        ...
        assert _to_dict(s) == [{"done": {"return": {"status": "ok", "code": 200}}}]
...
class TestRaisesAlias:
    ...
    def test_string_value(self):
        ...
        assert _to_dict(s) == [{"fail": {"raise": "something went wrong"}}]

    def test_dict_value(self):
        s = Steps().raises("fail", {"code": 404, "message": "not found"})
        assert _to_dict(s) == [
            {"fail": {"raise": {"code": 404, "message": "not found"}}}
        ]

    def test_expr_value(self):
        s = Steps().raises("fail", expr("e"))
        assert _to_dict(s) == [{"fail": {"raise": "${e}"}}]
...
class TestSwitchAlias:
    ...
    def test_with_next(self):
        ...
        assert _to_dict(s) == [
            {
                "check": {
                    "switch": [{"condition": "${x > 0}", "next": "positive"}],
                    "next": "fallback",
                }
            }
        ]
...
class TestLoopAlias:
    ...
    def test_loop_with_range(self):
        ...
        loop_body = _to_dict(s)[0]["loop"]["for"]
    ...
    def test_loop_with_index(self):
        ...
        assert _to_dict(s)[0]["loop"]["for"]["index"] == "idx"
    ...
    def test_equivalent_to_step(self):
        ...
        explicit = Steps().step(
            "loop", For(value="item", items=["a", "b", "c"], steps=inner)
        )
        assert alias.build() == explicit.build()
...
class TestParallelAlias:
    ...
    def test_with_exception_policy(self):
        ...
        assert _to_dict(s)[0]["p"]["parallel"]["exception_policy"] == "continueAll"

    def test_with_concurrency_limit(self):
        b1 = Steps().assign("x", a=1)
        b2 = Steps().assign("y", b=2)
        s = Steps().parallel("p", branches={"b1": b1, "b2": b2}, concurrency_limit=2)
        assert _to_dict(s)[0]["p"]["parallel"]["concurrency_limit"] == 2
...
class TestDoTryAlias:
    ...
    def test_try_with_steps_body(self):
        ...
        assert "steps" in _to_dict(s)[0]["safe"]["try"]
...
class TestNestedAlias:
    ...
    def test_without_next(self):
        ...
        assert "next" not in _to_dict(s)[0]["group"]
    ...
    def test_with_lambda(self):
        ...
        assert _to_dict(s)[0]["group"]["steps"] == [
            {"inner": {"assign": [{"x": 1}]}},
            {"done": {"return": "${x}"}},
        ]
